...
<div class="py-12">
    <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
        ...
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg mb-6">
            <div class="p-6 bg-white border-b border-gray-200">
                <div class="flex justify-between items-center">
                    ...
                    <a href="{{ route('transacciones.show', $transaccione) }}" 
        ...
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
            <form action="{{ route('transacciones.update', $transaccione) }}" method="POST" enctype="multipart/form-data" class="p-6">
                @csrf
                @method('PUT')
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <select name="tipo" id="tipo" required
                                ...
                                <option value="ingreso" {{ old('tipo', $transaccione->tipo) === 'ingreso' ? 'selected' : '' }}>Ingreso</option>
                                <option value="egreso" {{ old('tipo', $transaccione->tipo) === 'egreso' ? 'selected' : '' }}>Egreso</option>
                            </select>
                            @error('tipo')
                        ...
                        <div>
                            ...
                            <input type="date" name="fecha" id="fecha" value="{{ old('fecha', $transaccione->fecha->format('Y-m-d')) }}" required
                                   max="{{ date('Y-m-d') }}"
                        ...
                        <div>
                            ...
                            <div class="relative" x-data="contactoSearch({{ $transaccione->contacto_id ? json_encode([
                                'id' => $transaccione->contacto_id,
                                'nombre' => $transaccione->contacto->nombre ?? '',
                                'tipo' => ucfirst($transaccione->contacto->tipo ?? ''),
                                'email' => $transaccione->contacto->email ?? '',
                                'telefono' => $transaccione->contacto->telefono ?? '',
                                'texto_completo' => ($transaccione->contacto->nombre ?? '') . ' (' . ucfirst($transaccione->contacto->tipo ?? '') . ')',
                                'descripcion' => ($transaccione->contacto->telefono ?? '') . ' • ' . ($transaccione->contacto->email ?? '')
                            ]) : 'null' }})">
                                <input type="text" 
                                       x-model="search"
                                ...
                                <!-- Mensaje de "sin contacto" -->
                                <div x-show="showDropdown && search.length > 1 && resultados.length === 0"
                                     class="absolute z-10 mt-1 w-full bg-white shadow-lg rounded-md py-3 px-4 text-sm text-gray-500 ring-1 ring-black ring-opacity-5">
                                    No se encontraron contactos. 
                                    <a href="{{ route('contactos.create') }}" class="text-indigo-600 hover:text-indigo-500">Crear nuevo contacto</a>
                                </div>
                ...
                <!-- Referencia -->
                <div class="border-b border-gray-200 pb-6 mb-6">
                    <h3 class="text-lg font-medium text-gray-900 mb-4">Referencia</h3>
                    
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <!-- Tipo de Referencia -->
                        <div>
                            <label for="referencia_tipo" class="block text-sm font-medium text-gray-700">Tipo de Referencia *</label>
                            <select name="referencia_tipo" id="referencia_tipo" required
                                ...
                                <option value="">Seleccionar tipo</option>
                                <option value="obra" {{ old('referencia_tipo', $transaccione->referencia_tipo) === 'obra' ? 'selected' : '' }}>Obra</option>
                                <option value="producto" {{ old('referencia_tipo', $transaccione->referencia_tipo) === 'producto' ? 'selected' : '' }}>Producto</option>
                                <option value="servicio" {{ old('referencia_tipo', $transaccione->referencia_tipo) === 'servicio' ? 'selected' : '' }}>Servicio</option>
                                <option value="otro" {{ old('referencia_tipo', $transaccione->referencia_tipo) === 'otro' ? 'selected' : '' }}>Otro</option>
                            </select>
                        ...
                        <div>
                            <label for="referencia_nombre" class="block text-sm font-medium text-gray-700">Nombre de Referencia *</label>
                            <input type="text" name="referencia_nombre" id="referencia_nombre" value="{{ old('referencia_nombre', $transaccione->referencia_nombre) }}" required
                    ...
                    <!-- Datos adicionales de referencia (JSON) -->
                    <div class="mt-6">
                        <label class="block text-sm font-medium text-gray-700 mb-2">Datos Adicionales (Opcional)</label>
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                <label for="referencia_descripcion" class="block text-xs font-medium text-gray-600">Descripción</label>
                                <textarea name="referencia_datos[descripcion]" id="referencia_descripcion" rows="2"
                                          class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm"
                                          placeholder="Descripción detallada">{{ old('referencia_datos.descripcion', $transaccione->referencia_datos['descripcion'] ?? '') }}</textarea>
                            </div>
                            <div>
                                <label for="referencia_ubicacion" class="block text-xs font-medium text-gray-600">Ubicación</label>
                                <input type="text" name="referencia_datos[ubicacion]" id="referencia_ubicacion" value="{{ old('referencia_datos.ubicacion', $transaccione->referencia_datos['ubicacion'] ?? '') }}"
                                       class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm"
                ...
                <div class="border-b border-gray-200 pb-6 mb-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <select name="factura_tipo" id="factura_tipo" required
                                ...
                                <option value="">Seleccionar tipo</option>
                                <option value="manual" {{ old('factura_tipo', $transaccione->factura_tipo) === 'manual' ? 'selected' : '' }}>Manual</option>
                                <option value="archivo" {{ old('factura_tipo', $transaccione->factura_tipo) === 'archivo' ? 'selected' : '' }}>Archivo</option>
                            </select>
                            @error('factura_tipo')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        ...
                        <div>
                            ...
                            <input type="text" name="factura_numero" id="factura_numero" value="{{ old('factura_numero', $transaccione->factura_numero) }}"
                    ...
                    <!-- Datos manuales de factura (JSON) -->
                    <div id="datos-manuales" class="mt-6" style="display: none;">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                            <div>
                                ...
                                <input type="text" name="factura_datos[emisor]" id="factura_emisor" value="{{ old('factura_datos.emisor', $transaccione->factura_datos['emisor'] ?? '') }}"
                                       class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm"
                                       placeholder="Empresa emisora">
                            </div>
                            <div>
                                <label for="factura_receptor" class="block text-sm font-medium text-gray-700">Receptor</label>
                                <input type="text" name="factura_datos[receptor]" id="factura_receptor" value="{{ old('factura_datos.receptor', $transaccione->factura_datos['receptor'] ?? '') }}"
                                       class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm"
                            ...
                            <div>
                                ...
                                <input type="date" name="factura_datos[fecha_emision]" id="factura_fecha_emision" value="{{ old('factura_datos.fecha_emision', $transaccione->factura_datos['fecha_emision'] ?? '') }}"
                                       class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                            </div>
                        </div>
                            ...
                            <div class="border border-gray-200 rounded-lg overflow-hidden">
                                <table class="min-w-full divide-y divide-gray-200">
                                    ...
                                    <tbody id="conceptos-tbody" class="bg-white divide-y divide-gray-200">
                                        @php
                                            $conceptos = old('factura_datos.conceptos', $transaccione->factura_datos['conceptos'] ?? []);
                                        @endphp
                            ...
                            <div>
                                ...
                                <div class="mt-1 relative rounded-md shadow-sm">
                                    ...
                                    <input type="number" name="factura_datos[subtotal]" id="factura_subtotal" 
                                           value="{{ old('factura_datos.subtotal', $transaccione->factura_datos['subtotal'] ?? 0) }}"
                                           step="0.01" readonly
                            ...
                            <div>
                                ...
                                <div class="mt-1 relative rounded-md shadow-sm">
                                    ...
                                    <input type="number" name="factura_datos[impuestos]" id="factura_impuestos" 
                                           value="{{ old('factura_datos.impuestos', $transaccione->factura_datos['impuestos'] ?? 0) }}"
                                           step="0.01" min="0"
                            ...
                            <div>
                                ...
                                <div class="mt-1 relative rounded-md shadow-sm">
                                    ...
                                    <input type="number" name="factura_datos[total]" id="factura_total_manual" 
                                           value="{{ old('factura_datos.total', $transaccione->factura_datos['total'] ?? 0) }}"
                                           step="0.01" readonly
                                           class="pl-7 block w-full bg-gray-50 border-gray-300 rounded-md shadow-sm sm:text-sm">
                                </div>
                            </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <textarea name="factura_datos[condiciones]" id="factura_condiciones" rows="3"
                                          class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm"
                                          placeholder="Ej: Pago a 30 días, Contado, etc.">{{ old('factura_datos.condiciones', $transaccione->factura_datos['condiciones'] ?? '') }}</textarea>
                            </div>
                            <div>
                                ...
                                <textarea name="factura_datos[notas]" id="factura_notas" rows="3"
                                          class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm"
                                          placeholder="Observaciones, términos especiales, etc.">{{ old('factura_datos.notas', $transaccione->factura_datos['notas'] ?? '') }}</textarea>
                            </div>
                        </div>
                    </div>
                    ...
                    <div id="archivos-factura" class="mt-6" style="display: none;">
                        ...
                        <div class="mb-4">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-3" id="archivos-existentes">
                                @foreach($transaccione->factura_archivos as $index => $archivo)
                                <div class="border rounded-lg p-3 hover:bg-gray-50" data-archivo-index="{{ $index }}">
                                    <div class="flex items-center justify-between">
                                        ...
                                        <button type="button" 
                                                class="text-red-600 hover:text-red-900 eliminar-archivo" 
                                                data-archivo-index="{{ $index }}"
                        ...
                        <!-- Nuevos archivos -->
                        <div>
                            <label for="factura_archivos" class="block text-sm font-medium text-gray-700 mb-2">Agregar Nuevos Archivos</label>
                            <div class="relative">
                ...
                <div class="border-b border-gray-200 pb-6 mb-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                        ...
                        <div>
                            ...
                            <select name="metodo_pago_id" id="metodo_pago_id" required
                                ...
                                @foreach($metodosPago as $id => $nombre)
                                    <option value="{{ $id }}" {{ old('metodo_pago_id', $transaccione->metodo_pago_id) == $id ? 'selected' : '' }}>{{ $nombre }}</option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <input type="text" name="referencia_pago" id="referencia_pago" value="{{ old('referencia_pago', $transaccione->referencia_pago) }}"
                ...
                <div class="mb-6">
                    ...
                    <div>
                        ...
                        <textarea name="observaciones" id="observaciones" rows="3"
                                  ...
                                  placeholder="Notas adicionales...">{{ old('observaciones', $transaccione->observaciones) }}</textarea>
                        @error('observaciones')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
...
<script>
// Función para el autocompletado de contactos con contacto preseleccionado
function contactoSearch(contactoInicial = null) {
    return {
        search: contactoInicial ? contactoInicial.texto_completo : '',
        resultados: [],
        showDropdown: false,
        selectedId: contactoInicial ? contactoInicial.id : '',
        selectedContacto: contactoInicial,
        searchTimeout: null,
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Si está en modo manual y no hay conceptos, agregar uno
    if (facturaTypo.value === 'manual') {
        const tbody = document.getElementById('conceptos-tbody');
        if (tbody.children.length === 0) {
            agregarConcepto();
        }
    ...
    document.addEventListener('click', function(e) {
        if (e.target.closest('.eliminar-archivo')) {
            const button = e.target.closest('.eliminar-archivo');
            const index = button.dataset.archivoIndex;
            
            if (confirm('¿Estás seguro de que quieres eliminar este archivo?')) {
                fetch(`/transacciones/{{ $transaccione->id }}/archivo/${index}`, {
                    method: 'DELETE',
                    headers: {
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        const archivoElement = document.querySelector(`[data-archivo-index="${index}"]`);
                        if (archivoElement) {
...
function calcularTotales() {
    ...
    // Actualizar subtotal
    document.getElementById('factura_subtotal').value = sumaSubtotales.toFixed(2);
...
function calcularTotal() {
    ...
    document.getElementById('factura_total_manual').value = total.toFixed(2);
